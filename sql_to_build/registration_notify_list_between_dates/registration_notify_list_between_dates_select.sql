...
FROM change_between_dates
       ...
       JOIN clean_latest_address_staging AS latest_address
            ON change_between_dates.nhs_number = latest_address.nhs_number
       LEFT JOIN is_deceased_from_timeline on change_between_dates.nhs_number = is_deceased_from_timeline.nhs_number
WHERE nhs_clean_staging.nhs_deceased = '0'
  AND is_deceased_from_timeline.is_deceased IS null
  AND change_between_dates.has_access_to_essential_supplies = 'YES'