...
def build_sql(template_file, output_file, insert_delimiter='@') -> None:
    ...
    for line in file_to_lines(template_file):
        ...
        leading_statment = line_split_on_delimiter[0].strip()
        if leading_statment != '':
        ...
        end_sequence_of_line = files_to_insert[-1].strip().split(' ', 1)
        files_to_insert[-1] = end_sequence_of_line[0]

        for file_to_insert in files_to_insert:

            output_lines.extend(file_to_lines(file_to_insert, working_dir))
